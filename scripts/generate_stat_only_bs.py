...
parser.add_argument('-bsstat', metavar='bs_stat_file', help='output file with base station statistics (gziped)', required=True)
...
output_file = gzip.open(args.bsstat, 'w')
...
for key in bs_stat:
    ...
    output_file.write(str(bs) + '\t' + str(ks) + '\t' + str(ds) + '\t' + str(int(a)) + '\t' + str(pk0) + '\t' + str(pk1) + '\t' + str(pk2) + '\t' + str(pk3) + '\t' + str(pk4) + '\t' + str(pk5) + '\n')
