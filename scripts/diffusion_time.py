...
parser.add_argument("step", help="display a square of a given number",type=int)
...
for line in sms_file:
    ...
    tr = geoVoisin(geo1sms, tsms-step, tsms+step) #liste des packers
    ...
    if tRes != -1:
        tRes = tRes-tsms
    output_file.write(str(tsms) + '\t' + str(usms) + '\t' + str(vsms) + '\t' + str(diff1) + '\t' + str(geo1sms) + '\t' + str(tRes) + '\t' + str(tResGeo) + '\t' + str(len(tr)) + '\t' + str(len(de)) + '\t' + str(somme) + '\t' + str(de_geo_number) + '\t' + str(bs_packers_number)  + '\t' + str(bs_packers_alone_number) + '\t' + str(geo2sms) + '\n')
